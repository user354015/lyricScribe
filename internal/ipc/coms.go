...
func FindActivePlayer(conn *dbus.Conn, prefPlayers []string) (string, error) {
	...
	if len(players) == 0 {
		return "", shared.ErrNoActivePlayers
	}

	for pref := range prefPlayers {
		for i := range players {
			if strings.Contains(players[i], prefPlayers[pref]) {
...
func GetTrackInfo(conn *dbus.Conn, playerService string) (*shared.Track, error) {
	...
	var trackInfo *shared.Track

	trackInfo = &shared.Track{
...
func WatchTrackChanges(conn *dbus.Conn, playerService string, callback func(*shared.Track)) {
